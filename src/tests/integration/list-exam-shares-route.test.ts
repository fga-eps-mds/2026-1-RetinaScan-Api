...
describe('GET /api/exams/:examId/shares (integration)', () => {
  ...
  it('deve retornar 401 se não estiver autenticado', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/api/exams/123/shares' });
  ...
  it('deve listar os compartilhamentos de um exame', async () => {
    ...
    const colega = await UsuarioBuilder.anUser().withTipoPerfil('MEDICO').build();
    ...
    const res = await app.inject({ method: 'GET', url: `/api/exams/${exame.id}/shares` });
